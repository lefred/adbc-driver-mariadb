...
def test_userpass_uri(
    ...
    driver_path: str,
    uri: str,  # mariadb://localhost:3306/db
    creds: tuple[str, str],
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        # pytest.param("tls=true", True, id="tls=true"),  # Docker MariaDB container uses a self-signed certificate that fails validation.
...
def test_uri_default_port(
    ...
    no_port_uri = f"mariadb://{username}:{password}@{mariadb_host}/{mariadb_database}"
...
def test_uri_missing_host_error(
    ...
    no_host_uri = f"mariadb://{username}:{password}@/{mariadb_database}"
...
def test_invalid_uri_format(
    ...
    with pytest.raises(
        adbc_driver_manager.dbapi.ProgrammingError,
        match="invalid MariaDB URI format",
    ):
        with adbc_driver_manager.dbapi.connect(
            driver=driver_path,
            db_kwargs={"uri": "mariadb://[invalid-format"},
        ):
            pass
...
def test_unix_socket_parentheses(
    ...
    driver_path: str,
    creds: tuple[str, str],
    mariadb_socket_path: str,  # /tmp/mariadb.sock
) -> None:
    """Tests socket connection using mariadb://user:pass@(/path/to/socket.sock)/db"""
    username, password = creds

    socket_uri = f"mariadb://{username}:{password}@({mariadb_socket_path})/db"
...
def test_basic_dsn_connection(
    ...
    """Test basic connection to MariaDB using DSN format."""
    with adbc_driver_manager.dbapi.connect(
        driver=driver_path,
        db_kwargs={"uri": dsn},
    ) as conn:
...
def test_unix_socket_dsn(
    ...
    driver_path: str,
    creds: tuple[str, str],
    mariadb_socket_path: str,  # Example: /tmp/mariadb.sock
) -> None:
    """Tests socket connection using native DSN format: user:pass@unix(/path/to/socket.sock)/db"""
    username, password = creds

    socket_dsn = f"{username}:{password}@unix({mariadb_socket_path})/db"
