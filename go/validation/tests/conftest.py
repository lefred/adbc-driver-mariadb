...
def pytest_addoption(parser):
    ...
    parser.addoption("--vendor-version", action="store", default="mariadb")


@pytest.fixture(scope="session")
def driver(request, pytestconfig) -> adbc_drivers_validation.model.DriverQuirks:
    return mariadb.get_quirks(pytestconfig.getoption("vendor_version"))
...
def driver_path(driver: adbc_drivers_validation.model.DriverQuirks) -> str:
    ...
    return str(
        Path(__file__).parent.parent.parent / f"build/libadbc_driver_mariadb.{ext}"
    )
