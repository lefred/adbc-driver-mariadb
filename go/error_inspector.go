...
import (
	...
	mariadbwire "github.com/go-sql-driver/mysql"
)

type MariaDBErrorInspector struct{}

// InspectError examines a MariaDB error and formats it as an ADBC error
// mariadb error codes: https://www.fromdual.com/mariadb-error-codes-and-messages
func (m MariaDBErrorInspector) InspectError(err error, defaultStatus adbc.Status) adbc.Error {
	status := defaultStatus

	var mariadbErr *mariadbwire.MySQLError
	if errors.As(err, &mariadbErr) {
		...
		if mariadbErr.SQLState[0] != 0 && status == defaultStatus {
			switch string(mariadbErr.SQLState[:2]) {
			case "02": // No data
				status = adbc.StatusNotFound
	...
	if mariadbErr != nil {
		adbcErr.SqlState = mariadbErr.SQLState
	}
	return adbcErr
}
